...
-(bool)updateShader:(unsigned long)index {
...
-(bool)setup:(std::vector<NSString *>)shaders {
    ...
    for(int k=0; k<shaders.size(); k++) {
        ...
        NSLog(@"%@",[shaders[k] stringByDeletingPathExtension]);
        
        _library.push_back([_device newLibraryWithFile:[[NSBundle mainBundle] pathForResource:[shaders[k] stringByDeletingPathExtension] ofType:@"metallib"] error:&error]);
...
-(id)initWithFrame:(CGRect)frame {
    ...
    if(self) {
        _frame = frame;
       if([self setup:{@"default.metallib"}]) return nil;
    }
    return self;
}


-(id<MTLCommandBuffer>)setupCommandBuffer:(unsigned long)mode {
