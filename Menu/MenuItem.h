...
-(void)text:(NSString *)text {
    ...
    if(text.length>range) {
        [self setStringValue:
            [[NSString alloc] initWithFormat:@"%@...",
                [text substringWithRange:NSMakeRange(0,range)],
                [text substringWithRange:NSMakeRange(text.length-range,range)]
            ]
        ];
    }
    else {
    ...
    */
    ...
    //self.frame = NSMakeRect(self.frame.origin.x,self.frame.origin.y,self.frame.size.width,50);
    //NSLog(@"%f",self.frame.size.height);
...
-(id)init:(NSString *)name :(std::vector<NSString *>)names :(double)value {
    ...
    if(self) {
        ...
        long num = names.size();
        for(int k=0; k<num; k++) {
            buttons.push_back([[NSButton alloc] initWithFrame:NSMakeRect(0,19*((num-k)-1),19,19)]);
            [buttons[k] setButtonType:NSButtonTypeRadio];
            [buttons[k] setTarget:self];
            [buttons[k] setAction:@selector(onDown:)];

            textFields.push_back([[TextField alloc] initWithFrame:NSMakeRect([buttons[0] bounds].size.width+5,19*((num-k)-1)+1,128,19)]);
        ...
        [item setFrame:NSMakeRect(19,0,[buttons[0] bounds].size.width+5+width+19,19*num)];
...
-(void)text:(NSString *)name {
    [textField text:name];
    [view setFrame:NSMakeRect(0,0,19+[checkbox bounds].size.width+3+[textField bounds].size.width+19,19)];
}
...
-(id)init:(NSString *)name :(double)value {
    ...
    if(self) {
        view = [[NSView alloc] init];
        checkbox = [[NSButton alloc] initWithFrame:NSMakeRect(19,-1,19,19)];
        ...
        textField = [[TextField alloc] initWithFrame:NSMakeRect(19+3+[checkbox bounds].size.width,1,128,19)];
        [textField text:name];
        [view setFrame:NSMakeRect(0,0,19+[checkbox bounds].size.width+3+[textField bounds].size.width+19,19)];
        [view addSubview:textField];
        [self setView:view];
...
-(void)text:(NSString *)name {
    [textField text:name];
    [view setFrame:NSMakeRect(0,0,19+[slider bounds].size.width+5+[textField bounds].size.width+19,19)];
}
-(void)on {
...
-(id)init:(NSString *)name :(double)min :(double)max :(double)value :(bool)label {
    ...
    if(self) {
        ...
        slider = [[NSSlider alloc] initWithFrame:CGRectMake(19,-1,(200-40),19)];
        ...
        if(isLabel) {
            textField = [[TextField alloc] initWithFrame:NSMakeRect(19+[slider bounds].size.width+5,1,128,19)];
            [textField text:name];
            [view setFrame:NSMakeRect(0,0,19+[slider bounds].size.width+5+[textField bounds].size.width+19,19)];
            [view addSubview:textField];
            [self setView:view];
            ...
            [view setFrame:NSMakeRect(0,0,19+[slider bounds].size.width+19,19)];
            [view addSubview:textField];
            [self setView:view];
        }
