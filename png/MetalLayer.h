...
class MetalLayer {
	...
	public:
		...
		virtual id<MTLCommandBuffer> setupCommandBuffer(long mode) {
		...
		virtual bool init(int width,int height,std::vector<NSString *> shaders={@"defalt.metallib"}, bool isGetBytes=false) {
			...
			for(int k=0; k<shaders.size(); k++) {                
				...
				this->_library.push_back([this->_device newLibraryWithFile:[NSString stringWithFormat:@"%@/%@",[[NSBundle mainBundle] resourcePath],shaders[k]] error:&error]);
